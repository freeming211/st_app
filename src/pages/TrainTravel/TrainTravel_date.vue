...
export default {
  ...
  methods: {
    ...
    onNext() {
      if (this.dayOne == "") {
        Toast({ message: "请选择出团时间", duration: 1500 });
        return;
      }
      let time = [];
      let sum = 0;
      ...
      if (sum == 0) {
        Toast({ message: "请添加出团人员", duration: 1500 });
        return;
      }

      if (sum > 10) {
        Toast({ message: "人数不得超过10人", duration: 1500 });
        return;
      }
      if(this.price_type == 2 || this.price_type == 4){
        for (let i = 0; i < this.personList.length; i++) {
          time.push(this.personList[i].num);
        }
      }else{
        time = [0,0,0,this.personList[0].num,0,this.personList[1].num]
      }

      let daRen = 0
      for(let i = 0;i<this.personList.length-1;i++){
        daRen += this.personList[i].num
      }

      console.log(daRen)
      if(daRen == 0 && this.personList[this.personList.length-1].num != 0){
        Toast({ message: "请选择一名成人陪伴儿童", duration: 1500 });
        return;
      }
      ...
      sessionStorage.setItem("price_type", this.price_type);
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.code == 200) {
            ...
            this.price_type = res.data.data.price_type;
            // 价格体系,新增0525
            if(this.price_type == 1 || this.price_type == 3){
              this.personList = [
                { name: "占铺人数", id: 0, num: 0 },
                { name: "儿童不占铺", id: 1, num: 0 },
              ];
            }else if (this.price_type == 2 || this.price_type == 4) {
              this.personList = [
